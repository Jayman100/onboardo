...
function processAvailability(countryName: string) {
  ...
  const visaFree = ngData[countryName]["visa-free"].map(
    (cnt: { iso: string }) => cnt.iso.toLowerCase()
  );

  const visaOnArrival = ngData[countryName]["visa-on-arrival"].map(
    (cnt: { iso: string }) => cnt.iso.toLowerCase()
  );

  const visaOnline = ngData[countryName]["visa-online"].map(
    (cnt: { iso: string }) => cnt.iso.toLowerCase()
  );

  const eta = ngData[countryName]["electronic-travel-authorization"].map(
    (cnt: { iso: string }) => cnt.iso.toLowerCase()
  );

  const visaRequired = ngData[countryName]["visa-required"].map(
    (cnt: { iso: string }) => cnt.iso.toLowerCase()
  );
...
function App() {
  ...
  // @ts-ignore
  const { highlightCountries, selectedCountries, countriesVisaStatus } =
    ...
    <div className="flex justify-center h-screen border border-black p-16 m-16">
      ...
      <div className="control-center">
        ...
        <button
          ...
          onClick={_handleCountryProcessing}
        >
          Check
        </button>
